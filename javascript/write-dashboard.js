...
function StartExam() {
	...
	const url = new URLSearchParams(query);
	const ID = url.get("User");
	location.href = "Online Exam/index.html?User=" + ID + "";
}
function GetList1(email) {
	var count = 0;
	let html = "";
	let div = "";
	console.log("Running");
	db.collection("EnrolledCourses")
		...
		.onSnapshot((users) => {
			users.forEach((element) => {
				console.log(element.id + "Running");
				displayMarks(element.id);
			});
		});
}
			...
			db.collection("EnrolledCourses")
				...
				.then((info) => {
					...
					document.querySelector(
						".sc"
					).innerHTML = `<span>Marks obtained: ${Totalscore.toFixed(
						0
					)}%</span>  <button class="btn btn-secondary" onclick="loadTest('${marks}')">View Test</button>`;
				});
		});
}
displayPersonal();
function loadTest(params) {
	console.log(params);
	window.location.href = "Online Exam/processTest2.html?User=" + params + "";
}
function logout() {
	auth.signOut().then(() => {
				...
				<div class="out">
					...
					<div class="title-of-course">
						...
						<span class="lecture-name"
							>Lecture: ${element.data().LectureModule}</span
						>
					</div>
				</div>
			</div>`;
